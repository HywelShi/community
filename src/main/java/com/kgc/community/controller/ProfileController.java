...
public class ProfileController {
    ...
    public String profile(@PathVariable(name="action") String action,
        ...
        }else if("leaveMsgs".equals(action)){
            model.addAttribute("section","leaveMsgs");
            model.addAttribute("sectionName","我的留言");
        }else{
            model.addAttribute("section","collections");
            model.addAttribute("sectionName","我的收藏");
        }
        PaginationDTO paginationDTO = questionService.listByUserId(user.getId(), page, size);
        System.out.println(paginationDTO.getQuestions().size());
